...
// estimateHeight(): 光流高度融合（加速度积分 + TOF 修正）
...
Vector attitudeEuler; // derived once per IMU sample and reused by control/telemetry
Vector attitudeBodyUp(0, 0, 1);
bool landed;
...
void estimate() {
	...
	if (attitude.valid()) {
		...
		attitudeBodyUp = Quaternion::rotateVector(Vector(0, 0, 1), attitude);
	} else {
		attitudeEuler.invalidate();
		attitudeBodyUp.invalidate();
	}
}
...
void applyAcc() {
	...
	// Brushed motors intentionally spin at idle while armed. That output is not
	// evidence of flight: preserve ON_GROUND until a takeoff transition clears
	// the per-arm ground-idle state or the airborne latch is set.
	bool armedGroundIdle = armed && assistedTakeoffGroundIdle && !flightWasAirborne;
	landed = (!motorsActive() || armedGroundIdle) &&
		abs(accFilteredNorm - ONE_G) < ONE_G * 0.1f;
...
Vector worldVelocityEstimate() {
	float yaw = attitudeEuler.z;
	return Vector(
		velocity.x * cosf(yaw) - velocity.y * sinf(yaw),
		velocity.x * sinf(yaw) + velocity.y * cosf(yaw),
		velocity.z);
}
...
void estimateHeight() {
	...
	bool sampleValid = tofHealthy && opticalFlowHeight > 0.05f && opticalFlowHeight < 6.0f;
	bool newHeightSample = sampleValid && tofSequence != lastHeightSequence;

	if (newHeightSample) {
		lastHeightSequence = tofSequence;
		float sampleDt = constrain(tofSampleDt, 0.01f, 0.20f);
		if (!heightValid) {
...
// Rotation fit for the standard sensor mounting. Optical-flow packets describe
// an integration window that ends before UART delivery, so use delayed rates.
float flowGyroCompPitch = -0.78f;
float flowGyroCompRoll = -0.77f;
float flowGyroDelayMs = 40.0f;
float flowVelocitySmoothing = 0.20f;
...
float flowSpikeRejectVelocity = 2.5f; // sensor sanity threshold, not a commanded speed limit
float flowArmMinThrottle = 0.12f;
float flowStationaryGyro = radians(8.0f);
// The combined flow/ToF module is mounted 24 mm ahead of the yaw center.
// During yaw the sensor moves sideways even when the aircraft center does not.
constexpr float FLOW_SENSOR_FORWARD_OFFSET_M = 0.024f;
...
bool flowBiasReady = false;
bool flowBiasFallbackActive = false;
uint32_t flowBiasSamples = 0;
...
void estimateHorizontalVelocity() {
	...
	bool airborneCandidate = armed && thrustTarget > flowArmMinThrottle &&
		sampleFresh && !heightRejected && (opticalFlowHeight > 0.08f || velocity.z > 0.03f);
	flowAirborne = airborneDelay.update(airborneCandidate);
	bool newSensorSample = sampleFresh && opticalFlowSequence != lastProcessedSequence;
	bool newFlowSample = flowAvailable && newSensorSample;
	...
	if (newSensorSample && !tiltRejected && opticalFlowHeight > FLOW_SENSOR_MIN_HEIGHT) {
		...
		flowRawBodyVel.x = -opticalFlowVelocityY; // 前向速度（+X=机头）
		flowRawBodyVel.y = -opticalFlowVelocityX; // 左向速度（+Y=左）
		flowCompBodyVel.x = flowRawBodyVel.x - flowGyroCompPitch * flowGyroBodyVel.x;
		// v_center = v_sensor - omega x r. With r=(+0.024, 0, 0), yaw
		// contributes +yawRate*0.024 to the sensor's body-left velocity.
		flowCompBodyVel.y = flowRawBodyVel.y - flowGyroCompRoll * flowGyroBodyVel.y -
			delayedRates.z * FLOW_SENSOR_FORWARD_OFFSET_M;
		flowSpikeRejected =
			abs(flowCompBodyVel.x) > flowSpikeRejectVelocity ||
			abs(flowCompBodyVel.y) > flowSpikeRejectVelocity;

		// Do not qualify ground stationarity from the optical-flow mean itself:
		// that mean is the sensor bias being estimated and differs by sensor unit.
		stationaryCandidate =
			!flowAirborne &&
			abs(rates.x) < flowStationaryGyro &&
			abs(rates.y) < flowStationaryGyro &&
			abs(rates.z) < flowStationaryGyro &&
			thrustTarget < flowArmMinThrottle;

		if (stationaryCandidate) {
			if (flowBiasSamples < 30) {
				flowBiasSamples++;
				float sampleWeight = 1.0f / flowBiasSamples;
				flowBias.x += (flowCompBodyVel.x - flowBias.x) * sampleWeight;
				flowBias.y += (flowCompBodyVel.y - flowBias.y) * sampleWeight;
			} else {
				flowBias.x += (flowCompBodyVel.x - flowBias.x) * flowBiasAdapt;
				flowBias.y += (flowCompBodyVel.y - flowBias.y) * flowBiasAdapt;
			}
			flowBiasReady = flowBiasSamples >= 30;
			if (flowBiasReady) flowBiasFallbackActive = false;
		} else if (!flowAirborne && !flowBiasReady) {
			flowBiasSamples = 0;
			flowBias = Vector();
		} else if (flowAirborne && !flowBiasReady) {
			// Some sensors cannot qualify XY flow at the launch surface, so a
			// ground-bias prerequisite would make POS_HOLD impossible forever.
			// Start with zero bias after sustained airborne flow; the normal
			// stationary learner can replace it after landing.
			flowBias = Vector();
			flowBiasSamples = 0;
			flowBiasReady = true;
			flowBiasFallbackActive = true;
		}
	...
	flowStationary = stationaryCandidate || (!flowAirborne && thrustTarget < flowArmMinThrottle * 0.5f);
	...
	Vector worldVel = worldVelocityEstimate();
